...
class ClotheEditScreen extends StatefulWidget {
  static const routeName = "/edit-clothe";

  @override
  _ClotheEditScreenState createState() => _ClotheEditScreenState();
}

class _ClotheEditScreenState extends State<ClotheEditScreen> {
  final _formKey = GlobalKey<FormState>();

  TextEditingController clotheID;

  TextEditingController clotheName;

  TextEditingController clotheDescription;

  TextEditingController clothePrice;

  TextEditingController clotheSize;

  TextEditingController clotheImageURL;

  @override
  Widget build(BuildContext context) {
    final Map parsedData = ModalRoute.of(context).settings.arguments as Map;
    final Clothes clothe = parsedData['clothe'];
    final int index = parsedData['index'];

    clotheID = TextEditingController(text: clothe.id.toString());
    clotheDescription = TextEditingController(text: clothe.description);
    clotheName = TextEditingController(text: clothe.name);
    clothePrice = TextEditingController(text: clothe.price.toString());
    clotheSize = TextEditingController(text: clothe.size);
    clotheImageURL = TextEditingController(text: clothe.imageUrl);
    return Scaffold(
      appBar: AppBar(
        title: Text("Edit"),
      ),
                    ...
                    ElevatedButton(
                        onPressed: () {
                          if (_formKey.currentState.validate()) {
                            ...
                            ClotheHelper.editMovie(clothe,  index);
                            Navigator.pop(context);
                          }
                        },
                        child: Text("Edit"))
